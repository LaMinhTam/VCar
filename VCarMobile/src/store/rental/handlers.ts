...
export function* getLesseeRentRequests(action: {
  ...
  try {
    ...
    const response: AxiosResponse<IListRentRequestResponse> = yield call(
      axiosPrivate.get,
      ENDPOINTS.GET_LESSEE_REQUESTS,
      {
...
export async function getContractByRequestId(id: string) {
  try {
    const response: AxiosResponse<IContractByIdResponse> =
      await axiosPrivate.get(ENDPOINTS.GET_CONTRACT_BY_REQUEST_ID(id));
    const {code, data} = response.data;
    if (code === 200) {
      return {success: true, data};
    }
...
export async function lesseeApproveHandover(
  ...
  try {
    const response: AxiosResponse<IVehicleHandoverResponse> =
      await axiosPrivate.put(ENDPOINTS.LESSEE_APPROVE_HANDOVER(handover_id), {
        ...digital_signature,
      });
    const {code, data} = response.data;
    if (code === 200) {
...
export async function lessorApproveReturn(
  ...
  try {
    const response: AxiosResponse<IVehicleHandoverResponse> =
      await axiosPrivate.put(ENDPOINTS.LESSOR_APPROVE_RETURN(handover_id), {
        ...digital_signature,
      });
    const {code, data} = response.data;
    if (code === 200) {
...
export async function createReview({
  ...
  try {
    const response = await axiosPrivate.post(ENDPOINTS.CREATE_REVIEW, {
      rental_contract_id,
      rating,
      comment,
    });
