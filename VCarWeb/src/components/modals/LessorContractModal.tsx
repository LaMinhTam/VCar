...
const LessorContractModal = ({ record }: {
    ...
    useEffect(() => {
        async function fetchUser() {
            try {
                const response = await axiosPrivate.get(`/users/${record?.lessee}`);
                    ...
                    <Row gutter={[0, 12]}>
                        ...
                        {record.rental_status === 'PENDING' && <Col span={24}>
                            <div className="flex items-center justify-end gap-x-3">
                                <Button type="primary" danger onClick={handleViewContract} loading={viewLoading}>View Contract</Button>
                            </div>
                        </Col>}
                    </Row>
                </div>
